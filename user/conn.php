...
$stmt2 = $conn->prepare("SELECT * FROM studentt WHERE Student_Fname = ?"); 
$studentFname = "Amal"; // Bind the parameter and execute the query
$stmt2->bind_param("s", $studentFname); $stmt2->execute(); 
// Get the result
$result = $stmt2->get_result(); // Fetch the data and output it
if ($row = $result->fetch_assoc()) { 
    $fname2 = $row['Student_Fname'];
